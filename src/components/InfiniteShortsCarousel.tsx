...
// ── Main Component ───────────────────────────────────────────────────────────
export default function InfiniteShortsCarousel(props) {
    const { videos, cardWidth, cardHeight, arrowColor } = props

    const [currentIndex, setCurrentIndex] = useState(0)
    const [isMuted, setIsMuted] = useState(false)
    // activeIndex = which card has user clicked Play on (-1 = none)
    ...
    const next = () => {
        if (length > 0) {
            ...
            setActiveIndex(-1) // reset play state on navigation
        }
    }
    const prev = () => {
        if (length > 0) {
    ...
    // Mute/Unmute the active iframe via postMessage
    useEffect(() => {
    ...
    if (length === 0) {
        return (
            ...
            >
                <h2>Please paste your YouTube Shorts Links!</h2>
            </div>
        )
    }
            ...
            <div style={carouselStyle}>
                {currentVideos.map((videoUrl, index) => {
                    ...
                    return (
                        ...
                        >
                            {/* ── THUMBNAIL (shown until user hits Play) ── */}
                            <AnimatePresence>
                                {!isPlaying && (
                                    <motion.div
                                        key="thumb"
                            ...
                            {/* ── IFRAME (shown only after Play is clicked) ── */}
                            {isPlaying && (
                                <motion.div
                                    key="iframe"
                                    initial={{ opacity: 0 }}
...
addPropertyControls(InfiniteShortsCarousel, {
    videos: {
        ...
        defaultValue: [
            "https://www.youtube.com/shorts/5PEv_a1kS2g",
            "https://www.youtube.com/shorts/3Dcw4rT67_4",
            "https://www.youtube.com/shorts/P5jYtE_eT9Q",
            "https://www.youtube.com/shorts/V-kLg-bQ0qg",
        ],
        title: "YouTube Links",
    },
    cardWidth: {
