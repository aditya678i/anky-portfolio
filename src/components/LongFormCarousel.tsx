...
export default function InfiniteLongsCarousel(props) {
    ...
    if (length === 0) {
        return (
            <div style={{ ...containerStyle, color: "white", textAlign: "center" }}>
        ...
        <div style={containerStyle}>
            ...
            <div style={carouselStyle}>
                {currentVideos.map((videoUrl, index) => {
                    ...
                    let scale = 1, x = 0, opacity = 1, zIndex = 10, rotateY = 0
                    if (absOffset === 1) {
                        scale = 0.8; x = offset * (cardWidth * 1.1)
                        opacity = 0.8; zIndex = 8; rotateY = offset * -20
                    } else if (absOffset === 2) {
                        scale = 0.6; x = offset * (cardWidth * 1.6)
                        opacity = 0.5; zIndex = 6; rotateY = offset * -35
                    } else if (absOffset > 2) {
                        scale = 0.4; x = Math.sign(offset) * (cardWidth * 1.9)
                        opacity = 0; zIndex = 0; rotateY = Math.sign(offset) * -45
                    }

                    const thumb = getThumbnail(videoUrl)
                    const embedUrl = getEmbedUrl(videoUrl, isMuted)

                    return (
                        <motion.div
                            key={index}
                            initial={false}
                            animate={{ scale, x, opacity: isVisible ? opacity : 0, zIndex, rotateY }}
            ...
            >
                <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke={arrowColor} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
...
const cardStyle: React.CSSProperties = {
    position: "absolute", borderRadius: "16px", overflow: "hidden",
    boxShadow: "0 30px 60px -15px rgba(0,0,0,0.8), 0 0 0 1px rgba(255,255,255,0.08)",
...
const muteButtonStyle: React.CSSProperties = {
    ...
    backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)",
    outline: "none", padding: 0,
    boxShadow: "0 10px 25px -5px rgba(0,0,0,0.5)", zIndex: 60,
}

addPropertyControls(InfiniteLongsCarousel, {
    videos: {
        type: ControlType.Array,
        control: { type: ControlType.String, placeholder: "Paste YouTube link here" },
        defaultValue: [
            "https://www.youtube.com/watch?v=dQw4w9WgXcQ",
    ...
    cardWidth: {
        ...
        defaultValue: 560, min: 300, max: 1200,
        title: "Video Width",
    },
    cardHeight: {
        type: ControlType.Number,
        defaultValue: 315, min: 200, max: 800,
        title: "Video Height",
    },
    arrowColor: {
